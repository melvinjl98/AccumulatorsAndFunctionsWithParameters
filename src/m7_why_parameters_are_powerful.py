...
def better_draw_circles(n):
    ...
    for k in range(21):
        circle = rg.Circle(center, 10 * k * n)
        circle.attach_to(window)
        window.render(0.05)
...
def run_test_even_better_draw_circles():
    even_better_draw_circles(15, 2, .1)
    even_better_draw_circles(13, 5, .025)
    even_better_draw_circles(9, 7, .25)


def even_better_draw_circles(c, p, r):
    window = rg.RoseWindow(400, 400)

    center = rg.Point(50 * p, 40 * p)
    for k in range(c):
        circle = rg.Circle(center, 10 * k)
        circle.attach_to(window)
        window.render(r)
